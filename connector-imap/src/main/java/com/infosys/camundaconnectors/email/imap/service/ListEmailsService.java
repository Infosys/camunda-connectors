...
import com.infosys.camundaconnectors.email.imap.utility.SortCondition;
import java.util.*;
...
public class ListEmailsService implements IMAPRequestData {
  ...
  IMAPResponse<?> listEmailsResponse;

  @Override
  public Response invoke(Store store, Folder folder) {
    LOGGER.debug("ListEmailsUsingIMAP process started");
    ...
    try {
      ...
      if (filters != null && !filters.isEmpty()) {
        ...
        LOGGER.info("Fetching filtered messages from folder.");
        msgArr = folder.search(st);
      } else {
        LOGGER.info("Fetching all messages from folder.");
      ...
      LOGGER.debug("ListEmailsUsingIMAP Process Completed");
    ...
    if (outputType.equalsIgnoreCase("headerDetails")) {
      listEmailsResponse =
          new IMAPResponse<>(headerDetails.stream().limit(maxResults).collect(Collectors.toList()));
    } else if (outputType.equalsIgnoreCase("messageIds")) {
      listEmailsResponse =
          new IMAPResponse<>(messageIdList.stream().limit(maxResults).collect(Collectors.toList()));
    }
