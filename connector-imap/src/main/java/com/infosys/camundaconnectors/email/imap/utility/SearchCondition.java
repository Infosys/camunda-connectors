 ...
 * from, subject, message number, messageID, sentDate or receivedDate
 */
public class SearchCondition {
  ...
  private SearchTerm parseFilterMap(String searchType, String searchValue) {
    ...
    if (searchType.contains("size")) return sizeTerm(searchValue);
      ...
      return messageIDTerm(searchValue);
        ...
        && (searchType.contains("date") || searchType.contains("time")))
      return receivedDateTerm(searchValue);
    else return null;
  }
  ...
  private ReceivedDateTerm receivedDateTerm(String searchValue) {
    String[] args = searchValue.split("\\s+", 2);
    if (args.length == 2 && args[0] != null && args[1] != null) {
      return new ReceivedDateTerm(getComparison(args[0]), getDate(((String) parseValue(args[1]))));
    }
    return null;
  }
